...
class TranslationJudge:
    ...
    def batch_assess_translations(
        ...
        def process_pair(
            pair_id: int, source_text: str, translated_text: str
        ) -> Dict[str, Any]:
            try:
        ...
        with ThreadPoolExecutor(max_workers=max_workers) as executor:
            ...
            for future in as_completed(future_to_index):
                ...
                print(
                    f"Completed {completed_count}/{total_count} - Pair {result['pair_id']}: {result['num_errors']} errors found"
                )

                # Save to CSV intermittently
                if len(pending_rows) >= batch_size:
                    self._append_results_to_csv(
                        output_csv_path, pending_rows, csv_headers
                    )
    ...
    def _convert_result_to_csv_rows(
    ...
    def _append_results_to_csv(
        self, csv_path: str, results: List[Dict[str, Any]], headers: List[str]
    ) -> None:
...
def get_pairs_from_csv(
    csv_path: str, source_column: str, translated_column: str
) -> List[Tuple[str, str]]:
    csv_path = Path(csv_path)
    df = pd.read_csv(csv_path)
    return list(zip(df[source_column].to_list(), df[translated_column].to_list()))
...
if __name__ == "__main__":
    ...
    for translation in []:
        translation_pairs = get_pairs_from_csv(
            csv_path=f"../data/{language}_transcription_results.csv",
            source_column="original_transcription",
            translated_column=translation,
        )

        judge_groq = TranslationJudge(
            client=groq_client,
            model=model,
        ...
        results = judge_groq.batch_assess_translations(
            translation_pairs=translation_pairs,
            output_csv_path=f"../data/{language}_{model}_{translation}_llm_judge.csv",
            batch_size=100,
            max_workers=10,
        )
