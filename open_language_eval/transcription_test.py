...
parser.add_argument(
    ...
    type=int,
    default=5,
    help="Number of audio samples to test"
)
parser.add_argument(
    "--play-audio", "-p",
    action="store_true",
    help="Play audio samples during testing"
)
parser.add_argument(
    "--provider",
    type=str,
...
dataset = load_dataset("facebook/voxpopuli", args.language, split="test", streaming=True, trust_remote_code=True)
...
print(f"SUMMARY")
print(f"{'='*60}")
print(f"Average WER across {num_samples} samples: {sum(wer_scores)/len(wer_scores):.2%}")
